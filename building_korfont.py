# Patched building.py with Korean font setup for matplotlib
...
def _set_korean_font():
    ...
    if hasattr(sys, "_MEIPASS"):
        ...
        for fname in ["malgun.ttf", "Malgun.ttf", "NanumGothic.ttf", "NotoSansCJKkr-Regular.otf"]:
            candidate_paths.append(os.path.join(fonts_dir, fname))
    win_fonts = r"C:\Windows\Fonts"
    for fname in ["malgun.ttf", "Malgun.ttf", "NanumGothic.ttf", "NotoSansCJKkr-Regular.otf"]:
        candidate_paths.append(os.path.join(win_fonts, fname))
    ...
    for path in candidate_paths:
        if os.path.exists(path):
            try:
                ...
                chosen_name = font_manager.FontProperties(fname=path).get_name()
                break
            except Exception:
                pass
    ...
    if chosen_name:
        ...
        rcParams["font.sans-serif"] = ["Malgun Gothic", "NanumGothic", "DejaVu Sans"]
    rcParams["axes.unicode_minus"] = False

_set_korean_font()

# ===== original code below (imports reordered a bit) =====
# 부문별 열관류율 데이터 (W/m²·K)
material_u_values = {
    "외벽": {
# [ECO2-OD 프로그램 기준: λ값(100mm 두께, 표면열저항 포함) → U값 변환]
        "일반 콘크리트 (1:2:4)": 4.64,
        "목재 (중간)": 1.35,
        "콘크리트 블록(경량)": 3.38,
        "시멘트모르타르 (1:3)": 4.46,
        "페놀폼1종": 0.21,
        "천연슬레이트": 4.55,
        "폴리에스테르 흡음 단열재 1급": 0.32,
        "분무식중밀도폴리우레탄폼 1종": 0.22,
# [별표1] 공동주택 거실 외벽, 외기에 직접 면하는 경우 기준값
        "중부1 외벽 (기준 0.150 이하)": 0.15,
        "중부2 외벽 (기준 0.170 이하)": 0.17,
        "남부 외벽 (기준 0.220 이하)": 0.22,
        "제주도 외벽 (기준 0.290 이하)": 0.29
    },
    "지붕": {
# [ECO2-OD 프로그램 기준: λ값(100mm 두께, 표면열저항 포함) → U값 변환]
        "일반 콘크리트 (1:2:4)": 4.64,
        "페놀폼1종": 0.21,
# [별표1] 최상층 거실 반자 또는 지붕, 외기에 직접 면하는 경우(공동주택 기준)
        "중부1 지붕 (기준 0.150 이하)": 0.15,
        "중부2 지붕 (기준 0.180 이하)": 0.18,
        "남부 지붕 (기준 0.250 이하)": 0.25,
        "제주도 지붕 (기준 0.250 이하)": 0.25
    },
    "바닥": {
        "일반 콘크리트 (1:2:4)": 4.64,
        "시멘트모르타르 (1:3)": 4.46,
        "압출법보온판 1호": 0.28,
 # [별표1] 최하층 거실 바닥, 외기에 직접 면하는 경우 – 바닥난방/비난방(공동주택 기준)        
        "바닥난방 (중부1, 기준 0.150 이하)": 0.15,
        "바닥난방 (중부2, 기준 0.170 이하)": 0.17,
        "바닥난방 (남부, 기준 0.220 이하)": 0.22,
        "바닥난방 (제주도, 기준 0.290 이하)": 0.29,
        "비난방 바닥 (중부1, 기준 0.200 이하)": 0.20,
        "비난방 바닥 (중부2, 기준 0.330 이하)": 0.33
    },
    "창호": {
# [별표 4] 창과 문의 단열성능 
        "단판 유리 단창 (목재/ 플라스틱, 6mm)": 5.3,
        "일반 복층 유리 (목재/플라스틱, 공기층 12mm)": 2.8,
        "일반 3중 유리 (목재/플라스틱, 공기층 12mm)": 2.1,
        "Low-E 복층 유리 (소프트코팅, 목재/플라스틱, 12mm)": 2.1,
        "Low-E 3중 유리 (소프트코팅, 목재/플라스틱, 12mm)": 1.6,
        "Low-E 3중 유리 (아르곤 주입, 목재/플라스틱, 12mm)": 1.5,
# [별표1] 창 및 문, 외기에 직접 면하는 경우 – 공동주택 기준
        "중부1 창호 (기준 0.900 이하)": 0.9,
        "중부2 창호 (기준 1.000 이하)": 1.0,
        "남부 창호 (기준 1.200 이하)": 1.2,
...
class CarbonEmissionEducationProgram:
    ...
    def calculate(self):
        try:
            ...
            region_standards = {
                "중부1": {"외벽": 0.15, "지붕": 0.15, "바닥난방": 0.15, "창호": 0.9},
                "중부2": {"외벽": 0.17, "지붕": 0.18, "바닥난방": 0.17, "창호": 1.0},
                "남부": {"외벽": 0.22, "지붕": 0.25, "바닥난방": 0.22, "창호": 1.2},
                "제주도": {"외벽": 0.29, "지붕": 0.25, "바닥난방": 0.29, "창호": 1.6}
            }
            ...
            for section in self.sections:
                ...
                carbon_emission = energy_consumption * 0.474
